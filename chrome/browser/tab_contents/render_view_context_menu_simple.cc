...
//This is a simplfied version of context menu modified from render_view_context_menu.cc
// For using on touch base devices.
...
// static
const size_t RenderViewContextMenuSimple::kMaxExtensionItemTitleLength = 75;
// static
const size_t RenderViewContextMenuSimple::kMaxSelectionTextLength = 50;

// static
bool RenderViewContextMenuSimple::IsDevToolsURL(const GURL& url) {
  return url.SchemeIs(chrome::kChromeDevToolsScheme) &&
      url.host() == chrome::kChromeUIDevToolsHost;
}

// static
bool RenderViewContextMenuSimple::IsInternalResourcesURL(const GURL& url) {
...
RenderViewContextMenuSimple::RenderViewContextMenuSimple(
...
void RenderViewContextMenuSimple::Init() {
  InitMenu();
  PlatformInit();
}
...
void RenderViewContextMenuSimple::AppendExtensionItems(
...
void RenderViewContextMenuSimple::RecursivelyAppendExtensionItems(
    const ExtensionMenuItem::List& items,
    bool can_cross_incognito,
    ui::SimpleMenuModel* menu_model,
    int *index) {
...
void RenderViewContextMenuSimple::SetExtensionIcon(const std::string& extension_id) {
  ExtensionService* service = profile_->GetExtensionService();
...
void RenderViewContextMenuSimple::AppendAllExtensionItems() {
...
void RenderViewContextMenuSimple::InitMenu() {
...
void RenderViewContextMenuSimple::LookUpInDictionary() {
  // Used only in the Mac port.
  NOTREACHED();
}

bool RenderViewContextMenuSimple::AppendCustomItems() {
...
void RenderViewContextMenuSimple::AppendImageItems() {
...
void RenderViewContextMenuSimple::AppendAudioItems() {
  AppendMediaItems();
  menu_model_.AddSeparator();
...
void RenderViewContextMenuSimple::AppendVideoItems() {
  AppendMediaItems();
  menu_model_.AddSeparator();
  menu_model_.AddItemWithStringId(IDC_CONTENT_CONTEXT_SAVEAVAS,
...
void RenderViewContextMenuSimple::AppendMediaItems() {
...
void RenderViewContextMenuSimple::AppendPluginItems() {
...
void RenderViewContextMenuSimple::AppendPageItems() {
...
void RenderViewContextMenuSimple::AppendCopyItem() {
  menu_model_.AddItemWithStringId(IDC_CONTENT_CONTEXT_COPY,
                                  IDS_CONTENT_CONTEXT_COPY);
}

void RenderViewContextMenuSimple::AppendSearchProvider() {
  DCHECK(profile_);
...
void RenderViewContextMenuSimple::AppendEditableItems() {
#if 0
...
void RenderViewContextMenuSimple::AppendSpellcheckOptionsSubMenu() {
...
void RenderViewContextMenuSimple::AppendBidiSubMenu() {
...
ExtensionMenuItem* RenderViewContextMenuSimple::GetExtensionMenuItem(int id) const {
...
bool RenderViewContextMenuSimple::IsCommandIdEnabled(int id) const {
  if (id == IDC_PRINT &&
...
bool RenderViewContextMenuSimple::IsCommandIdChecked(int id) const {
...
void RenderViewContextMenuSimple::ExecuteCommand(int id) {
  ...
  switch (id) {
    ...
    case IDC_CONTENT_CONTEXT_OPENIMAGENEWTAB:
    case IDC_CONTENT_CONTEXT_OPENAVNEWTAB:
      OpenURL(params_.src_url, NEW_FOREGROUND_TAB, PageTransition::LINK);
      break;
    ...
    case IDC_SAVE_PAGE: {
      const GURL& referrer =
          params_.frame_url.is_empty() ? params_.page_url : params_.frame_url;
      const GURL& url = params_.page_url;
      DownloadManager* dlm = profile_->GetDownloadManager();
      dlm->DownloadUrl(url, referrer, params_.frame_charset,
                       source_tab_contents_);
       break;
    ...
    case IDC_ZOOM_MINUS:
      source_tab_contents_->render_view_host()->Zoom(PageZoom::ZOOM_OUT);
      break;

    case IDC_ZOOM_NORMAL:
      source_tab_contents_->render_view_host()->Zoom(PageZoom::RESET);
      break;

    case IDC_ZOOM_PLUS:
      source_tab_contents_->render_view_host()->Zoom(PageZoom::ZOOM_IN);
      break;
    ...
    case IDC_CONTENT_CONTEXT_LANGUAGE_SETTINGS:
// FIXME: We need ot implement a QT version FontsLanguagesWindow instead of hack in toolkit independent files.
...
void RenderViewContextMenuSimple::MenuClosed() {
  source_tab_contents_->render_view_host()->ContextMenuClosed(
      params_.custom_context);
}

bool RenderViewContextMenuSimple::IsDevCommandEnabled(int id) const {
...
string16 RenderViewContextMenuSimple::PrintableSelectionText() {
...
void RenderViewContextMenuSimple::OpenURL(
    const GURL& url,
...
void RenderViewContextMenuSimple::CopyImageAt(int x, int y) {
  source_tab_contents_->render_view_host()->CopyImageAt(x, y);
}

void RenderViewContextMenuSimple::Inspect(int x, int y) {
...
void RenderViewContextMenuSimple::WriteURLToClipboard(const GURL& url) {
...
void RenderViewContextMenuSimple::MediaPlayerActionAt(
